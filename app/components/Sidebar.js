  ...
  return (
    <>
      ...
      >
        <div className="sticky top-0 space-y-8 bg-white">
          <div className="h-20 flex items-center px-4 border-b md:px-8">
            <Link href="/" className="flex-none">
              <img src="/images/MotionUI_logo.png" width={150} className="mx-auto" />
            </Link>
          </div>
        ...
        <button
          className={`absolute top-4 right-4 p-3 bg-gray-900 text-white rounded-full shadow-lg focus:outline-none sm:hidden ${
            !sidebarOpen ? "block" : "hidden"
          }`}
      ...
      <button
        className={`hidden sm:block fixed z-50 top-4 right-4 p-3 bg-gray-900 text-white rounded-full shadow-lg focus:outline-none ${
          !sidebarOpen ? "block" : "hidden"
        }`}
        onClick={toggleSidebar}
      >
